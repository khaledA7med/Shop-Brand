...
import { ControlValueAccessor, NG_VALUE_ACCESSOR, NG_VALIDATORS, NgControl, AbstractControl, Validator, ValidationErrors } from "@angular/forms";
...
@Directive({
	selector: "input[formControlName][InputMask], input[formControl][InputMask], input[ngModel][InputMask]",
	providers: [DecimalPipe, CURRENCYDIRECTIVE_VALUE_ACCESSOR, CURRENCYDIRECTIVE_VALIDAT0R],
})
export class InputMaskDirective implements ControlValueAccessor, Validator, OnInit {
	...
	private readonly allowedKeys: string[] = ["Backspace", "Tab", "End", "Home", "Delete", "Del", "Ctrl", "ArrowLeft", "ArrowRight", "Left", "Right"];
	...
	private isNumberTooLarge(rawInputValue: string): boolean {
		...
		if (this.max !== undefined && this.max !== null && value > this.max) return value > this.max;
	...
	private isAllowedKeyWithControl(event: KeyboardEvent): boolean {
		if (event.ctrlKey || event.metaKey) {
			return (
				this.allowedKeysUsedWithControl.includes(event.key) || this.allowedKeysUsedWithControl.map((key) => key.toUpperCase()).includes(event.key)
			);
		} else {
			return false;
		}
	...
	writeValue(value: number): void {
		...
		this.reformatInput(+value?.toFixed(2)!);
